...
本 Skill 用一套**4 层主线流程 + 8 道过程质检 + 跨文档漂移双向复检**的工程化框架解决这些问题，把短剧创作做成可追踪、可回滚、可交付的流水线，而不是灵光一现的手工作坊。
...
- **8 道过程质检**：设定 / 结构 / 大纲 / 策划通读 / 剧本 / 衔接 / 整剧通读 / 分镜，每道都带 P0 硬阻断
...
| `/分镜脚本 {起止集}` | 全剧剧本完成并通过 `/整剧通读` 后，把定稿剧本转成可执行 AI 视频分镜 |
...
### 模式四：滚动批次生产
`/分集剧本 {起止集}` + `/剧本质检 {起止集}` 按批次循环推进；全剧剧本完成、批次剧本质检覆盖全剧并通过 `/整剧通读` 后，才进入正式 `/分镜脚本 {起止集}`。`deliveryProgress` 字段自动记账剧本完成区间、正式分镜完成区间与下一批次。
...
恢复现场时**必须同时**检查：剧本完成区间、批次剧本质检覆盖、`/整剧通读` 状态、正式分镜缺口、下一批次、最近一批质检状态——不允许只看 `currentStep` 就继续写。
