...
所有产物保存在当前项目目录。下面这棵树表示项目完整跑通后的最终形态，不代表启动时就要一次性创建全部文件：
...
1. 先检查 `剧本状态.json` 是否存在；不存在时按新项目处理。
2. 若已有内容，先恢复 `currentStep`，只检查当前步骤和前置步骤真正需要的文件，不预建未来步骤产物。
3. 若 `currentStep=开始`，优先从 `选题分析.md` 的推荐版中恢复最近一轮候选方向、当前方向理解和待确认项。
4. 若已有旧项目内容但缺少当前步骤继续所必需的前置文档，只补建当前所需的最小文档集，不顺手创建后续阶段文件。
5. 任何阶段发现前置文档不成立，回退到前一阶段重做，不带病推进。
6. 项目启动一律先走 `/开始`，不要绕过 `/开始` 直接进入 `/立项`。
7. 若已进入滚动生产，恢复现场时**必须同时完成以下四项检查**，不允许只看 `currentStep` 就继续写：
   - 检查 `deliveryProgress.scriptCompletedRanges`：确认最近已完成的剧本批次范围
   - 检查 `deliveryProgress.storyboardCompletedRanges`：确认是否存在"剧本已完成但分镜未完成"的缺口
...
默认采用"什么时间用，什么时间创建"的策略。不要为了看起来完整而预先创建未来步骤的空文件或目录。
...
| `/分镜脚本 {起止集}` | `references/commands/storyboard.md` | 把已通过剧本质检的单集转成可执行 AI 视频分镜 |
